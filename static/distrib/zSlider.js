...
function Peppermint (_this, options) {
  ...
  function changeActiveSlide (n, speed) {
    ...
    colorLog(`[{()}]~~~~~CustomEvent-sliderPage~~~`, '#52264a', _this.querySelector('section').className)
    let save = new CustomEvent('sliderPage', {
      cancelable: true,
      detail: {
        class: _this.querySelector('section').className,
        SlidesNum: slidesNumber,
        page: n
      }
    })
    ...
    EventBurrito(slideBlock, {
      ...
      move: function (event, start, diff, speed) {
        ...
        if (diff.x) {
          ...
          var skip = Math.floor(ratio) + (ratio - Math.floor(ratio) > 0.25 ? 1 : 0)
          ...
          var flick = diff.time < flickThreshold + flickThreshold * skip / 1.8 && Math.abs(diff.x) - skip * slider.width > (skip ? -slider.width / 9 : 20)
  ...
  function setup () {
    ...
    var dotsTarget = o.dotsContainer || _this
    ...
    if (_this.offsetWidth === 0) {
      ...
      whide = _this.offsetWidth
    }
    slider.width = _this.clientWidth
...
function EventBurrito (_this, options) {
  ...
  var eventModel = (support.pointerEvents ? 1 : (support.msPointerEvents ? 2 : 0))
